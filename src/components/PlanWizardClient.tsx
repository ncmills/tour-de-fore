...
export default function PlanWizardClient() {
  ...
  // Restore wizard state after magic link verification
  useEffect(() => {
    if (searchParams.get("verified") === "1" && !hasAutoGenerated.current) {
      try {
        const saved = localStorage.getItem("tdf-wizard-state");
        if (saved) {
          const parsed = JSON.parse(saved) as WizardState;
          Object.entries(parsed).forEach(([key, value]) => {
            dispatch({ type: "SET_FIELD", field: key as keyof WizardState, value });
          });
          localStorage.removeItem("tdf-wizard-state");
          hasAutoGenerated.current = true;
          // Auto-generate after state is restored
          setTimeout(() => {
            const btn = document.getElementById("tdf-generate-btn");
            if (btn) btn.click();
          }, 500);
        }
      } catch {}
  ...
  const handleSendMagicLink = async () => {
    ...
    if (!state.organizerEmail.includes("@")) {
    ...
    try {
      // Save wizard state to localStorage before sending magic link
      localStorage.setItem("tdf-wizard-state", JSON.stringify(state));

      const res = await fetch("/api/auth/send-link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: state.organizerEmail, returnTo: "/plan-a-trip?verified=1" }),
      });
          ...
          {magicLinkSent ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: "center" }}>
              ...
              <p style={{ fontSize: "0.9rem", color: "rgba(255,255,255,0.45)", marginBottom: "1.5rem" }}>
                We sent a verification link to <strong style={{ color: "rgba(255,255,255,0.7)" }}>{state.organizerEmail}</strong>. Click it to create your account and generate your plan.
              </p>
              <button
            ...
            <>
              <motion.button
                id="tdf-generate-btn"
                onClick={searchParams.get("verified") === "1" ? handleGenerate : handleSendMagicLink}
                disabled={sendingLink}
                whileHover={{ scale: 1.01 }}
                whileTap={{ scale: 0.99 }}
                style={{ width: "auto", padding: "1.3rem 3rem", background: "rgba(220,38,38,0.9)", color: "#fff", border: "none", borderRadius: 4, fontSize: "1rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", cursor: sendingLink ? "not-allowed" : "pointer", opacity: sendingLink ? 0.6 : 1, fontFamily: "var(--font-plan-block), sans-serif", margin: "0 auto", display: "block" }}
              >
                {sendingLink ? "Sending..." : "Unleash the Devils"}
              </motion.button>
              <p style={{ fontSize: "0.84rem", color: "rgba(255,255,255,0.25)", textAlign: "center", marginTop: "1rem" }}>
                1 free plan per month — we&apos;ll verify your email to save your trips.
              </p>
            </>
          )}
        </Question>
