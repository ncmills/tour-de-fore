...
export default function PlanWizardClient() {
  ...
  // Handle error/expired states from verify redirect
  useEffect(() => {
    ...
    if (authStatus === "expired") {
      setCurrentQ(6);
      setRevealedCount(7);
      setError("Your verification link expired. Enter your email and we'll send a new one.");
    }
    if (errorStatus === "limit") {
      setError("You've used your free plan this month. Check back next month!");
    }
    if (errorStatus === "generate") {
  ...
  const handleSendMagicLink = async () => {
    ...
    try {
      const res = await fetch("/api/auth/send-link", {
        ...
        body: JSON.stringify({ email: state.organizerEmail, wizardState: state }),
      });
          ...
          {magicLinkSent ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: "center" }}>
              ...
              <p style={{ fontSize: "0.9rem", color: "rgba(255,255,255,0.45)", marginBottom: "1.5rem" }}>
                We sent a verification link to <strong style={{ color: "rgba(255,255,255,0.7)" }}>{state.organizerEmail}</strong>. Click it and we&apos;ll generate your plan automatically.
              </p>
              <div style={{ display: "flex", gap: "1.5rem", justifyContent: "center", flexWrap: "wrap" }}>
                <button
                  onClick={handleSendMagicLink}
                  disabled={sendingLink}
                  style={{ background: "none", border: "1px solid rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.5)", fontSize: "0.85rem", cursor: "pointer", padding: "0.6rem 1.2rem", borderRadius: 4, opacity: sendingLink ? 0.5 : 1 }}
                >
                  {sendingLink ? "Sending..." : "Resend email"}
                </button>
                <button
