...
export default async function ShareableTripPage({ params, searchParams }: Props) {
  ...
  return <ShareableTripClient plan={plan} planId={id} selectedOptions={stored.selectedOptions} selectedTier={selectedTier} timing={stored.inputs ? { tripMonth: stored.inputs.tripMonth, preferredSeason: stored.inputs.preferredSeason, flexible: stored.inputs.flexible } : null} />;
}
