...
export async function POST(req: NextRequest) {
  try {
    ...
    await sendEmail({
      ...
      html: `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 500px; margin: 0 auto; padding: 40px 30px; text-align: center;"><h1 style="font-size: 24px; font-weight: 300; color: #c87941; margin: 0 0 24px;">Tour de Fore</h1><p style="color: #555; margin-bottom: 32px;">Click below to reset your password.</p><a href="${link}" style="display: inline-block; background: #c87941; color: #fff; text-decoration: none; padding: 14px 40px; border-radius: 4px; font-size: 14px; letter-spacing: 0.1em; text-transform: uppercase;">Reset Password</a><p style="color: #999; font-size: 12px; margin-top: 32px;">This link expires in 15 minutes.</p></div>`,
      critical: true,
    });
